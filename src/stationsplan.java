 ...
 */
@WebServlet(name = "stationsplan", 
			urlPatterns = {"/stationsplan/*"}
		)
public class stationsplan extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.indexOf("/stationsplan") != -1) {
				int startpos = requesturi.indexOf("/stationsplan");
...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
				...
				if(name.equals("OBJ_Stationsplan_Bodenindikatorart")) {
					if(wert.equals("Hinweisfeld_und_Leitstreifen")) {
						merkmaleJsonObject.put("bodenindikatorart", "Leitstreifen zum und Hinweisfeld vor Plan");
					} else if(wert.equals("Leitstreifen_zum_Plan")) {
						merkmaleJsonObject.put("bodenindikatorart", "Leitstreifen zum Plan");
					} else if(	wert.equals("Hinweisfeld_vor_Plan")) {
						merkmaleJsonObject.put("bodenindikatorart", "Hinweisfeld vor Plan");
					} else if(wert.equals("nein")) {
						merkmaleJsonObject.put("bodenindikatorart", "keine Bodenindikatoren");
					} else {
						NVBWLogger.warning("in Servlet " + this.getServletName() 
						+ " unerwarteter Wert bei Merkmal Name '" + name + "'" 
						+ ", Wert '" + wert + "'");
					}
				} else if(name.equals("OBJ_Stationsplan_Laufzeit_sek_D2134"))
					merkmaleJsonObject.put("laufzeit_sek", (int) Double.parseDouble(wert));
				else if(name.equals("OBJ_Stationsplan_Lon"))
					merkmaleJsonObject.put("lon", Double.parseDouble(wert));
				else if(name.equals("OBJ_Stationsplan_Lat"))
					merkmaleJsonObject.put("lat", Double.parseDouble(wert));
				else if(name.equals("OBJ_Stationsplan_Fahrplanakustisch"))
					merkmaleJsonObject.put("planakustisch", wert.equals("true"));
				else if(name.equals("OBJ_Stationsplan_Plantaktil"))
					merkmaleJsonObject.put("plantaktil", wert.equals("true"));
				else if(name.equals("OBJ_Stationsplan_Foto"))
					merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
				else if(name.equals("OBJ_Stationsplan_2_Foto"))
					merkmaleJsonObject.put("objekt2_Foto", Bild.getBildUrl(wert, dhid));
				else
					NVBWLogger.warning("in Servlet " + this.getServletName() 
