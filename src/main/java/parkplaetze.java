...
public class parkplaetze extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String selectHaltestelleSql = "SELECT CASE WHEN NOT korr.merkmal_id IS NULL THEN 'korrektur' ELSE 'erfassung' END AS quelle, "
			...
			+ "erf.objekt_id, erf.osmids, "
            + "CASE WHEN osm_id IS NULL THEN o_lon ELSE osm_lon END as lon, "
            + "CASE WHEN osm_id IS NULL THEN o_lat ELSE osm_lat END as lat, "
            + "erf.gemeinde, erf.ortsteil, osmimportiert, "
			+ "erf.infraidtemp, "
			...
			+ "osm.id as osm_id, "
			+ "osm.osmid AS osmids, "
			+ "ST_X(o.koordinate) AS o_lon, "
			+ "ST_Y(o.koordinate) AS o_lat, "
            + "ST_X(osm.koordinate) AS osm_lon, "
            + "ST_Y(osm.koordinate) as osm_lat, "
