 ...
 */
@WebServlet(name = "verkaufsstelle", 
			urlPatterns = {"/verkaufsstelle/*"}
		)
public class verkaufsstelle extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.contains("/verkaufsstelle")) {
				int startpos = requesturi.indexOf("/verkaufsstelle");
...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
                ...
                switch (name) {
                    case "OBJ_Verkaufsstelle_Art_D1021" -> merkmaleJsonObject.put("verkaufsstellenart", wert);
                    case "OBJ_Verkaufsstelle_stufenfrei_D1022" ->
                            merkmaleJsonObject.put("stufenfrei", wert.equals("true"));
                    case "OBJ_Verkaufsstelle_Vorhanden_D1020" -> {
                        //nichts zu tun
                    }
                    case "OBJ_Verkaufsstelle_Foto" ->
                            merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Verkaufsstelle_EingangzuVerkaufsstelle_Foto" ->
                            merkmaleJsonObject.put("eingang_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Verkaufsstelle_Oeffnungszeiten_Foto" ->
                            merkmaleJsonObject.put("oeffnungszeiten_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Verkaufsstelle_WegzuVerkaufsstelle_Foto" ->
                            merkmaleJsonObject.put("wegzu_Foto", Bild.getBildUrl(wert, dhid));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		LOG.info("Request angekommen in /verkaufsstelle doPost ...");

		response.setCharacterEncoding("UTF-8");
		JSONObject ergebnisJsonObject = new JSONObject();
		ergebnisJsonObject.put("status", "fehler");
		ergebnisJsonObject.put("fehlertext", "POST Request /verkaufsstelle ist nicht vorhanden");
		response.getWriter().append(ergebnisJsonObject.toString());
