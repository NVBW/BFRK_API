 ...
 */
@WebServlet(name = "toilette", 
			urlPatterns = {"/toilette/*"}
		)
public class toilette extends HttpServlet {
	private static final long serialVersionUID = 1L;

	private static final Logger LOG = NVBWLogger.getLogger(toilette.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if((bfrkConn == null) || !bfrkConn.isValid(5)) {
				...
				init();
				if((bfrkConn == null) || !bfrkConn.isValid(5)) {
					LOG.severe("es konnte keine DB-Verbindung herstellt werden, in toilette doGet");
			...
			LOG.severe("SQLException aufgetreten in toilette doGet, " + e1.toString());
			JSONObject ergebnisJsonObject = new JSONObject();
			ergebnisJsonObject.put("status", "fehler");
			ergebnisJsonObject.put("fehlertext", "keine DB-Verbindung verfügbar, bitte Administrator informieren: "
					+ e1.toString());
			...
			LOG.severe("IOException aufgetreten in toilette doGet, " + e1.toString());
			JSONObject ergebnisJsonObject = new JSONObject();
			ergebnisJsonObject.put("status", "fehler");
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.contains("/toilette")) {
				int startpos = requesturi.indexOf("/toilette");
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
                ...
                switch (name) {
                    case "OBJ_Toilette_Rollstuhltauglich" -> merkmaleJsonObject.put("rollstuhltauglich", wert);
                    case "OBJ_Toilette_Lokalschluessel_Notiz" -> merkmaleJsonObject.put("lokalschluesselnotiz", wert);
                    case "OBJ_Toilette_Oeffnungszeiten_Beschreibung_D1075" ->
                            merkmaleJsonObject.put("oeffnungszeitenbeschreibung", wert);
                    case "OBJ_Toilette_Foto" -> merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Toilette_LokalerSchluessel_Foto" ->
                            merkmaleJsonObject.put("lokalerschluessel_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Toilette_Nutzungsbedingungen_Foto" ->
                            merkmaleJsonObject.put("nutzungsbedingungen_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Toilette_Oeffnungszeiten_Foto" ->
                            merkmaleJsonObject.put("oeffnungszeiten_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Toilette_Umgebung1_Foto" ->
                            merkmaleJsonObject.put("umgebung1_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Toilette_Umgebung2_Foto" ->
                            merkmaleJsonObject.put("umgebung2_Foto", Bild.getBildUrl(wert, dhid));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		LOG.info("Request angekommen in /toilette doPost ...");
