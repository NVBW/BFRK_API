...
public class leihradanlage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.indexOf("/leihradanlage") != -1) {
...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
				...
				if(name.equals("OBJ_Leihradanlage_Art"))
					merkmaleJsonObject.put("art", wert);
				else if(name.equals("OBJ_Leihradanlage_Notizen"))
					merkmaleJsonObject.put("notiz", wert);
				else if(name.equals("OBJ_Leihradanlage_lon"))
					merkmaleJsonObject.put("lon", Double.parseDouble(wert));
				else if(name.equals("OBJ_Leihradanlage_lat"))
					merkmaleJsonObject.put("lat", Double.parseDouble(wert));
				
				else if(name.equals("OBJ_Leihradanlage_Foto"))
					merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
				else if(name.equals("OBJ_Leihradanlage_Kontaktdaten_Foto"))
