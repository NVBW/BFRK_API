 ...
 */
@WebServlet(name = "tuer", 
			urlPatterns = {"/tuer/*"}
		)
public class tuer extends HttpServlet {
	private static final long serialVersionUID = 1L;

	private static final Logger LOG = NVBWLogger.getLogger(tuer.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.contains("/tuer")) {
				int startpos = requesturi.indexOf("/tuer");
...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
			...
			while(selectMerkmaleRS.next()) {
                ...
                switch (name) {
                    case "OBJ_Tuer_Art_D2032" -> {
                        if (wert.equals("drehfluegeltuer")) {
                            merkmaleJsonObject.put("tuerart", "Drehflügeltür");
                        } else if (wert.equals("karusseltuer")) {
                            merkmaleJsonObject.put("tuerart", "Rotationstür");
                        } else if (wert.equals("pendeltuer")) {
                            merkmaleJsonObject.put("tuerart", "Pendeltür");
                        } else if (wert.equals("schiebetuer")) {
                            merkmaleJsonObject.put("tuerart", "Schiebetür");
                        } else {
                    ...
                    case "OBJ_Tuer_Oeffnungsart_D2033" -> merkmaleJsonObject.put("oeffnungsart", wert);
                    case "OBJ_Tuer_Breite_cm_D2034" ->
                            merkmaleJsonObject.put("lichtebreite_cm", (int) Double.parseDouble(wert));
                    case "OBJ_Tuer_Vorhanden_D2030" -> {
                        //nichts zu tun
                    }
                    case "OBJ_Tuer_Foto" -> merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		LOG.info("Request angekommen in /tuer doPost ...");

		response.setCharacterEncoding("UTF-8");
		JSONObject ergebnisJsonObject = new JSONObject();
		ergebnisJsonObject.put("status", "fehler");
		ergebnisJsonObject.put("fehlertext", "POST Request /tuer ist nicht vorhanden");
		response.getWriter().append(ergebnisJsonObject.toString());
