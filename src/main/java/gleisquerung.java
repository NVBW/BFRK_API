 ...
 */
@WebServlet(name = "gleisquerung", 
			urlPatterns = {"/gleisquerung/*"}
		)
public class gleisquerung extends HttpServlet {
	private static final long serialVersionUID = 1L;

	private static final Logger LOG = NVBWLogger.getLogger(gleisquerung.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.contains("/gleisquerung")) {
				int startpos = requesturi.indexOf("/gleisquerung");
...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
                ...
                switch (name) {
                    case "OBJ_Gleisquerung_Breite_cm" ->
                            merkmaleJsonObject.put("breite_cm", (int) Double.parseDouble(wert));
                    case "OBJ_Gleisquerung_Verbindungsfunktion" -> merkmaleJsonObject.put("verbindungsfunktion", wert);
                    case "OBJ_Gleisquerung_Foto" -> merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Gleisquerung_Weg1_Foto" ->
                            merkmaleJsonObject.put("richtung1_Foto", Bild.getBildUrl(wert, dhid));
                    case "OBJ_Gleisquerung_Weg2_Foto" ->
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		LOG.info("Request angekommen in /gleisquerung doPost ...");
