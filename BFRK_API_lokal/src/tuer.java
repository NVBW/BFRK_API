...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
				...
				if(name.equals("OBJ_Tuer_Art_D2032")) {
				...
				} else if(name.equals("OBJ_Tuer_Oeffnungsart_D2033"))
					merkmaleJsonObject.put("oeffnungsart", wert);
				else if(name.equals("OBJ_Tuer_Breite_cm_D2034"))
					merkmaleJsonObject.put("lichtebreite_cm", (int) Double.parseDouble(wert));
				else if(name.equals("OBJ_Tuer_Vorhanden_D2030")) {
					//nichts zu tun
				}
				else if(name.equals("OBJ_Tuer_Foto"))
