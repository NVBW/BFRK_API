 ...
 */
@WebServlet(name = "informationsstelle", 
			urlPatterns = {"/informationsstelle/*"}
		)
public class informationsstelle extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.indexOf("/informationsstelle") != -1) {
				int startpos = requesturi.indexOf("/informationsstelle");
...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
				...
				if(name.equals("OBJ_Infostelle_Art_D1031"))
					merkmaleJsonObject.put("infostellenart", wert);
				else if(name.equals("OBJ_Infostelle_Stufenfrei_D1032"))
					merkmaleJsonObject.put("stufenfrei", wert.equals("true"));
				
				else if(name.equals("OBJ_Infostelle_Foto"))
					merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
				else if(name.equals("OBJ_Infostelle_EingangzuInfostelle_Foto"))
					merkmaleJsonObject.put("eingang_Foto", Bild.getBildUrl(wert, dhid));
				else if(name.equals("OBJ_Infostelle_Oeffnungszeiten_Foto"))
					merkmaleJsonObject.put("oeffnungszeiten_Foto", Bild.getBildUrl(wert, dhid));
				else if(name.equals("OBJ_Infostelle_WegzuInfostelle_Foto"))
