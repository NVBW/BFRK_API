...
public class gleisquerung extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("dhid") != null) {
			...
			if(requesturi.indexOf("/gleisquerung") != -1) {
...
//		String selectHaltestelleSql = "SELECT * FROM merkmal_view WHERE objekt_id = ?;";
		...
		try {
			...
			while(selectMerkmaleRS.next()) {
				...
				if(name.equals("OBJ_Gleisquerung_Breite_cm"))
					merkmaleJsonObject.put("breite_cm", (int) Double.parseDouble(wert));
				else if(name.equals("OBJ_Gleisquerung_Verbindungsfunktion"))
					merkmaleJsonObject.put("verbindungsfunktion", wert);
				
				else if(name.equals("OBJ_Gleisquerung_Foto"))
					merkmaleJsonObject.put("objekt_Foto", Bild.getBildUrl(wert, dhid));
				else if(name.equals("OBJ_Gleisquerung_Weg1_Foto"))
					merkmaleJsonObject.put("richtung1_Foto", Bild.getBildUrl(wert, dhid));
				else if(name.equals("OBJ_Gleisquerung_Weg2_Foto"))
